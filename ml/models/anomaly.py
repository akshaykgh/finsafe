...
def _build_reason(row, df):
    reason_parts = []

    cat_avg = df[df['category'] == row['category']]['amount'].mean()
    if cat_avg > 0 and row['amount'] > cat_avg * 2:
        reason_parts.append(f"{round(row['amount'] / cat_avg, 1)}x your usual {row['category']} spend")

    user_hour_mean = df['hour_parsed'].mean()
    user_hour_std  = df['hour_parsed'].std() if df['hour_parsed'].std() > 0 else 1
    tx_hour        = pd.to_datetime(row['date'], errors='coerce').hour
    if abs(tx_hour - user_hour_mean) / (user_hour_std + 1e-9) > 2:
        reason_parts.append(f"outside your normal hours (usually around {int(user_hour_mean)}:00)")
    ...
    if day_count > avg_daily * 2:
        reason_parts.append(f"{day_count} transactions today vs your average {round(avg_daily, 1)}/day")
...
def detect_anomalies(df):
    ...
    df['anomaly_reason'] = [
        _build_reason(row, df) if row['anomaly'] else ''
